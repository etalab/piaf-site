...
* `yarn` to install npm packages listed in `package.json`
* `yarn run start` to compile the site with development settings and run BrowserSync

## Build your site
* `yarn run build:dev` to compile the site with development settings
* `yarn run build` to compile the site for production
...
Note: By default, Netlify uses `jekyll build` as the build command. Make sure to change this to `yarn run build`.
